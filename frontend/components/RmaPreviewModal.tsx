...
const DeviceTable = ({ devices, rma }: { devices: Device[], rma: Rma }) => (
    <table className="w-full text-sm border-collapse border border-gray-300">
        ...
        <tbody>
            <SafeData data={devices} fallback={<tr><td colSpan={5} className="p-4 text-center text-slate-500">No devices listed for this RMA.</td></tr>}>
                {devices.map(device => {
                    ...
                    return (
                        <tr key={device.serialNumber} className="border-b">
                            <td className="border p-2">{device.model ?? 'N/A'}</td>
                            <td className="border p-2">{device.partNumber ?? 'N/A'}</td>
    ...
    return (
        <Page>
            ...
            <Section title="Service Details">
                <h3 className="text-base font-bold text-black mt-4">{device.model ?? 'N/A'}</h3>
                ...
                {closedCycles.length > 0 ? closedCycles.map(cycle => {
                    ...
                    return (
                        <button key={cycle.creationDate} onClick={() => onSelect(device, cycle)} className="w-full text-left p-4 border rounded-lg hover:bg-slate-100 flex justify-between items-center">
                            <div>
                                <p className="font-bold">{device.model} (S/N: {device.serialNumber})</p>
...
export const RmaPreviewModal: React.FC<RmaPreviewModalProps> = ({ rma, onClose }) => {
  ...
  const PdfDownloadButton = () => (
      !pdfFileName ? null :
    <PDFDownloadLink document={<RmaPdfDocument rma={rma} selectedCycle={selectedReport?.cycle} />} fileName={pdfFileName}>
      {({ loading }) => (
