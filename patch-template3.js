...
content = content.replace(
  ...
  `<input className="flex-1 rounded-md border-slate-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm" placeholder="Test Name" value={newStep.name} onChange={e => setNewStep({...newStep, name: e.target.value})} />
                            <input className="flex-[2] rounded-md border-slate-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm" placeholder="Question / Detail" value={newStep.question} onChange={e => setNewStep({...newStep, question: e.target.value})} />`
);
